...
def train(model, optimizer, loss_function, loader, device, epoch, encoding_size):
    model.train()
    total_loss = 0
    mse_sum = 0
    num_correct = 0
...
def complete_training(model, optimizer, loss_function, loader, device, n_epochs, name, encoding_size, save=False):

    training_history = pd.DataFrame(columns=['epoch', 'loss', 'mse', 'avg_output_vector', 'avg_label_vector'])

    for epoch in range(1, n_epochs+1):
        loss, avg_label_vector, avg_output_vector, avg_correct, mse = train(model, optimizer, loss_function, loader, device, epoch, encoding_size=encoding_size)
